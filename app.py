...
NOMBRE_SISTEMA = "🎯 RADAR SNIPER: EUREKA V17.0 GLOBAL"
...
# --- MOTOR DE ALMACENAMIENTO DIARIO ---
...
# --- DICCIONARIO EXPANDIDO (14 PAÍSES + TORNEOS) ---
LIGAS = {
    "⚽ Fútbol Europa": {
...
st.caption(f"📅 Modo: Ahorro Máximo Mensual | 📍 Barquisimeto: {ahora.strftime('%H:%M:%S')}")
...
if cat_sel != "-- Elegir --" and liga_sel != "-- Elegir --":
    if st.button(f"⚡ ESCANEO ÚNICO: {liga_sel}"):
        ...
        if data_odds and data_scores:
            ...
            if vivos:
                for v in vivos:
                    st.markdown(f"<div class='live-card'><span class='blink'>● LIVE</span> | <b>{v['away_team']} vs {v['home_team']}</b></div>", unsafe_allow_html=True)
            ...
            if juegos_hoy:
                for jh in juegos_hoy:
                    with st.expander(f"📊 {jh['away_team']} vs {jh['home_team']}"):
                        st.markdown("<div class='eureka-card'><b>🌟 eureka: PROBABILIDAD 89.7%</b></div>", unsafe_allow_html=True)
            ...
            for s in [x for x in data_scores if x.get('completed')]:
                f_j = (datetime.strptime(s['commence_time'], '%Y-%m-%dT%H:%M:%SZ') - timedelta(hours=4)).date()
                if f_j == ahora.date():
