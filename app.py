...
def obtener_analisis_profundo(nombre, liga_id, es_jugador=False):
    ...
    # Cálculo Final: Ponderación Pulcra
    restante = 1.0 - p5_w
...
# --- EL SELECTOR DE VALOR ABSOLUTO (FILTRO EUREKA ÚNICO) ---
def selector_elite_eureka(juego, liga_id):
    ...
    for market in juego['bookmakers'][0]['markets']:
        ...
        # Micro-mercados (Corners, Props, etc.)
...
# --- LLAMADA API (RECUPERADA CON TODOS LOS MERCADOS) ---
def fetch_api_blindado(l_id, tipo_endpoint):
    ...
    if clave_memoria in st.session_state.cache_maestro:
        return st.session_state.cache_maestro[clave_memoria], "BÓVEDA-S34", "🛡️"

    # Solicitamos mercados principales y secundarios para la búsqueda "en el rincón"
    m_list = "h2h,spreads,totals,soccer_corners,soccer_cards,player_points,player_rebounds"
    url = f"https://api.the-odds-api.com/v4/sports/{l_id}/odds/?apiKey={KEYS[0]}&regions=us&markets={m_list}"
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            return data, res.headers.get('x-requests-remaining', '0'), 1
    except: pass
    return None, 0, 0
...
st.caption(f"🛡️ Blindaje Activo | Motor Universal Multi-Mercado | 📍 {ahora.strftime('%H:%M:%S')}")
...
if cat_sel != "-- Elegir --" and liga_sel != "-- Elegir --":
    if st.button(f"🎯 INICIAR ANÁLISIS ELITE EUREKA"):
        ...
        odds, creds, _ = fetch_api_blindado(l_id, "odds")
                ...
                if eureka_final:
                    with st.expander(f"✅ DETECCIÓN: {j['away_team']} @ {j['home_team']}", expanded=True):
                        st.markdown(f"""<div class='eureka-card'>
                            ...
                            Este mercado ha sido seleccionado como el de mayor valor entre todas las opciones disponibles (ML, Totales, Corners, Props).
                            </p>
